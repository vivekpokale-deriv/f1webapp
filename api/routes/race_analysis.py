...
@race_analysis_bp.route('/race-pace-plot/<int:year>/<race>', methods=['GET'])
def get_race_pace_plot(year, race):
    """
    Get race pace comparison plot.
    ...
    Returns:
        Image: Race pace plot image
    """
    try:
        # Get optional parameters
        num_drivers = request.args.get('drivers', default=10, type=int)
        
        logger.info(f"Creating race pace plot for {year} {race} (top {num_drivers} drivers)")
        
        # Use the shared race_analysis_service instance
        session = race_analysis_service.get_session(year, race, 'R')
        
        # Create the plot
        _, filename = race_analysis_service.create_race_pace_plot(session, num_drivers)
        
        # Return the image file
        return send_file(filename, mimetype='image/png')
        
    except Exception as e:
        logger.error(f"Error creating race pace plot: {e}")
        return error_response(f"Error creating race pace plot: {str(e)}", 500)


@race_analysis_bp.route('/team-pace-plot/<int:year>/<race>', methods=['GET'])
def get_team_pace_plot(year, race):
    """
    Get team pace comparison plot.
    ...
    Returns:
        Image: Team pace plot image
    """
    try:
        logger.info(f"Creating team pace plot for {year} {race}")
        
        # Use the shared race_analysis_service instance
        session = race_analysis_service.get_session(year, race, 'R')
        
        # Create the plot
        _, filename = race_analysis_service.create_team_pace_plot(session)
        
        # Return the image file
        return send_file(filename, mimetype='image/png')
        
    except Exception as e:
        logger.error(f"Error creating team pace plot: {e}")
        return error_response(f"Error creating team pace plot: {str(e)}", 500)


@race_analysis_bp.route('/lap-sections-plot/<int:year>/<race>/<session>', methods=['GET'])
def get_lap_sections_plot(year, race, session):
    """
    Get lap sections analysis plot.
    ...
    try:
        ...
        logger.info(f"Creating lap sections plot for {year} {race} {session} {driver_list}")
        
        # Use the shared race_analysis_service instance
        session_obj = race_analysis_service.get_session(year, race, session)
        
        # Create the plot
        _, filename = race_analysis_service.create_lap_sections_plot(session_obj, driver_list)
        
        # Return the image file
        return send_file(filename, mimetype='image/png')
        
    except Exception as e:
        logger.error(f"Error creating lap sections plot: {e}")
        return error_response(f"Error creating lap sections plot: {str(e)}", 500)
